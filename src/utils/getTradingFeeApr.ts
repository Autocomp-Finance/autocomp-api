...
export const getTradingFeeApr = async (
  ...
  try {
    ...
    for (const pairDayData of pairDayDatas) {
      ...
      pairAddressToAprMap[pairAddress] = new BigNumber(
...
export const getTradingFeeAprSushi = async (
  ...
  try {
    let queryResponse0 = await client.query({
      query: pairDayDataSushiQuery(
        addressesToLowercase(pairAddresses),
        start0,
        end0
      ),
    });
    ...
    const pairDayDatas0 = queryResponse0.data.pairs.map(
      (pair) => pair.dayData[0]
    );
    const pairDayDatas1 = queryResponse1.data.pairs.map(
      (pair) => pair.dayData[0]
    );
...
export const getYearlyPlatformTradingFees = async (
  ...
  try {
    ...
    const dailyVolumeUSD = new BigNumber(
      data.data.uniswapDayData.dailyVolumeUSD
    );

    yearlyTradingFeesUsd = dailyVolumeUSD
      .times(liquidityProviderFee)
      .times(365);
  } catch (e) {
