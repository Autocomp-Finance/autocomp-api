...
// abis
import SushiComplexRewarderTime from "../../../abis/sushi/SushiComplexRewarderTime.json";
...
const getFarmApys = async (params: MiniChefApyParams) => {
  ...
  const miniChefTokenPerSecond = new BigNumber(
    await minichefContract.methods[
  ...
  const miniChefTokenPrice = await fetchPrice({
    oracle,
  ...
  if (rewarderConfig) {
    ...
    rewarderTokenPerSecond = new BigNumber(
  ...
  for (let i = 0; i < pools.length; i++) {
    ...
    const miniChefYearlyRewardsInUsd = miniChefYearlyRewards
      ...
      .dividedBy(DECIMALS);
    totalYearlyRewardsInUsd = totalYearlyRewardsInUsd.plus(
      miniChefYearlyRewardsInUsd
    );
    ...
    if (rewarderConfig) {
      ...
      const yearlyNativeRewards = nativeRewards
        .dividedBy(secondsPerBlock)
        .times(secondsPerYear);
      const nativeRewardsInUsd = yearlyNativeRewards
        .times(rewarderTokenPrice)
        .dividedBy(DECIMALS);
      totalYearlyRewardsInUsd =
        totalYearlyRewardsInUsd.plus(nativeRewardsInUsd);
    }
